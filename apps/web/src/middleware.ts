...
const renewThresholdSeconds = (): number => {
...
const attachSilentRenewCookie = async (res: NextResponse, jwt: VerifiedJwt): Promise<NextResponse> => {
    const nowSec = Math.floor(Date.now() / 1000);
    const threshold = renewThresholdSeconds();
    const remaining = jwt.exp !== undefined ? jwt.exp - nowSec : 0;
    const shouldRenew = threshold <= 0 || jwt.exp === undefined || remaining <= threshold;
    if (!shouldRenew)
        return res;
...
const readJwtCookie = async (rawCookie: string | undefined): Promise<VerifiedJwt> => {
    if (!rawCookie)
