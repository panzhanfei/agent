...
import { findOwnedConversation, listConversationMessages, toModelHistory, } from "@fambrain/db";
...
export const GET = async (_request: Request, context: {
    params: Promise<{
        id: string;
    }>;
}) => {
    ...
    try {
        const conversation = await findOwnedConversation(parsedId.data, session.userId);
        if (!conversation) {
