...
await assertCase("重复敲字 normalize 后按单字短路", async () => {
  if (normalizeIntakeUtterance("呢呢呢？？？") !== "呢？") {
    throw new Error(
      `normalize 失败: ${normalizeIntakeUtterance("呢呢呢？？？")}`
    );
  }
...
await assertCase("pipeline：多问 retrieve → composite", async () => {
  ...
  const { decision, earlyExit } = await runIntakePipeline({
    ...
    intakeHistory: [
      { role: "user", content: "我叫什么？今年多大？做过哪些项目？" },
    ],
  });
  if (earlyExit) {
  ...
  if (
    ...
    throw new Error(
      `期望 slots≥2，实际 ${decision.routeMode}/${decision.compositeSlots?.length ?? 0}`
    );
  }
});
...
await assertCase(
  "repeat：REPEAT_QUESTION_CACHE_DISABLED=1 时不命中",
  async () => {
...
const entitySwapRetrieveJson = JSON.stringify({
    intent: "retrieve_and_answer",
    searchQuery: "友谊时光 入职 年份 哪一年 工作经历",
    subTasks: ["友谊时光入职年份"],
    topics: ["experience"],
    language: "zh",
    confidence: 0.9,
...
await assertCase("pipeline：示例 6c JSON → default 单槽（非 enumeration）", async () => {
    const history: DbChatTurn[] = [
        { role: "user", content: "我在哪一年入职奥卡云的？" },
        {
            role: "assistant",
            content: "你在2021年入职西安奥卡云科技有限公司。",
        },
        { role: "user", content: "友谊时光呢" },
    ];
    const { decision, earlyExit } = await runIntakePipeline({
        intakeRaw: entitySwapRetrieveJson,
        userQuestion: "友谊时光呢",
        intakeHistory: history,
    });
    ...
    const enumSlot = (decision.compositeSlots ?? []).find(
        (s) => s.queryType === "enumeration"
    );
    if (enumSlot) {
        throw new Error(
            `示例 6c 禁止 enumeration 槽: ${enumSlot.label}`
        );
    }
    if (!/友谊时光|入职|年份/.test(decision.searchQuery)) {
        throw new Error(`searchQuery 应含友谊时光/入职/年份: ${decision.searchQuery}`);
    }
});
...

{
  ...
  const checkEntitySwap = (
    result: Awaited<ReturnType<typeof runIntakePipeline>>
  ) => {
    const { decision, earlyExit } = result;
    if (earlyExit || decision.intent !== "retrieve_and_answer") {
      throw new Error(
        `期望 retrieve 非早退，earlyExit=${earlyExit} intent=${decision.intent}`
      );
    ...
    if (/enumeration/i.test(blob) && !/入职|年份|哪一年/.test(blob)) {
      throw new Error(`不应走经历列举，实际: ${blob.slice(0, 240)}`);
    }
    if (!/云联智慧/.test(blob)) {
      throw new Error(`plan/slots 应含云联智慧: ${blob.slice(0, 240)}`);
    }
    if (!/入职|年份|哪一年|时间线|时间段|任职/.test(blob)) {
      throw new Error(`应继承入职年份检索意图: ${blob.slice(0, 240)}`);
    }
    const enumSlot = slots.find((s) => s.queryType === "enumeration");
    if (enumSlot) {
      throw new Error(
        `实体替换续问禁止 enumeration 槽: ${enumSlot.label} / ${enumSlot.searchQuery}`
      );
    }
  };
  try {
    ...
    for (let attempt = 1; attempt <= 2; attempt += 1) {
      try {
        if (attempt > 1) console.log("    … live 重试（小模型抖动）");
        checkEntitySwap(await runLiveLikeIntakeNode(history));
        console.log(
          "  ✓ 实体替换续问「云联智慧呢」→ 非 enumeration，含入职/年份"
        );
        lastErr = null;
        break;
      } catch (e) {
    ...
    if (lastErr) {
      ...
      console.error(
        `  ✗ 实体替换续问「云联智慧呢」→ 非 enumeration，含入职/年份: ${msg}`
      );
      process.exitCode = 1;
    }
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    console.error(`  ✗ 实体替换续问「云联智慧呢」: ${msg}`);
    process.exitCode = 1;
  }
}
