...
await assertAsync("空 hits → insufficientEvidence", async () => {
  ...
  if (!/没有检索到|未检索到姓名|No relevant content|No name was found/i.test(r.answer)) {
...
await runStreamCase(
  ...
  ({ result, assistantTexts }) => {
    ...
    if (assistantTexts.length !== 1) {
      throw new Error(
        `应单次 assistant 输出，实际 ${assistantTexts.length} 段`
      );
    }
  }
);
...
await runStreamCase(
  "英文空 hits",
  emptyHitsInput({ language: "en" }),
  ({ result }) => {
    if (!result.insufficientEvidence)
      throw new Error("应 insufficientEvidence");
    if (!/No relevant content|No name was found/i.test(result.answer)) {
