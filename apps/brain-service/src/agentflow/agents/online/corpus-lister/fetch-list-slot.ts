 ...
 *
 * @param slot - Intake 派生的列举槽（含 enumerationControl、enumerationPage、executor=list_corpus）
 * @param corpusUserId - 语料归属用户，`data/doc/users/<id>/corpus/` 根路径
 * @param asOfDate - 可选基准日（YYYY-MM-DD）；配合 timeWindowYears 过滤经历时间窗
 */
export const fetchListSlot = async (
    ...
    /** project | experience；优先信 enumerationControl，否则按 topics/label 结构化推断 */
    const listKind =
