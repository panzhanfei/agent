...
| P0-15 | Analyst | 同问「**我叫什么 年龄 职业 从业经历**」→ 一次答 **赵一 / 28 岁 / 秦汉新城智慧园林**（语料无此人），一次答 **潘展飞** + 简历引用（正确） | KM hits 波动 + Analyst 在 weak hits 下用训练数据填「完整简历模板」；复合问法单 queryType 单检索 | **Intake retrievalPlan≥1** 主路由 + 动态槽 KM + merge；代码只编译 plan（空→clarify）；Analyst 禁推算年龄、enumeration 逐条列 | ✅ **已解决**（2026-06 · `verify:r6-no-cache` 综合履历 3 遍）← §2.5.3 |
...
### 2.10 Intake 档 B：主路径规划 + 旁路纠偏（✅ P0-31 · 2026-07）

> **背景：** P0-30 去掉口语二次规划后，入口仍堆了「调用前盲合并短续问」「散文 clarify 当指代信号」等结构启发式，复盘时分不清是 **LLM 工单错了** 还是 **代码又规划了一遍**。本轮把 Intake 定型为：**主逻辑 = 任务规划（语义终稿）**；**代码 = 兜底 + 纠偏**。
...
| `guards/intake-continuation-guard.ts` | **恒 noop**（指代消解靠 intake-node JSON merge + LLM 重试） |
...
**Golden 脚本定位：** G1～G5c + **GMem**（8 项）；`GOLDEN_RUNS=3` 连跑目标 **≥6/8×3**（G1～G5 核心 ≥4/5）。P0-13～16 专项脚本 + Golden 双重验收。
...
#### 2.5.3 P0-15 / R6-3 — composite 分槽检索（✅ 2026-06 · 2026-07 档 B 修订）

**完整路由（Intake 主信号 + 编译层，2026-07 修订）：**
...
**路由结论：** 凡 `retrieve_and_answer` 须 LLM `retrievalPlan≥1` → **`routeMode=slots`**（1～N）。空 plan → clarify。Analyst / KM 按槽数分支。

**单问 ↔ 多问（LLM 负责，代码不二次规划）：**

| 场景 | LLM 应输出 | 代码行为 |
|------|------------|----------|
| 单问 | **1 项** plan | 编译为 1 槽 `intake_retrieval_plan` |
| 多问并列 | **N 项** plan（含编号实体各 1 项） | 编译为 N 槽 |
| plan 空 / 解析失败 | — | **clarify**（`defaultIntakeDecision` 或 composite 早退） |
| stale inherited plan | LLM 应在本轮重写 plan | `query-signals` 仅诊断；**不**在 guard 收束/展开 |

`hasExplicitMultipartStructure` / `hasStaleMultipartFromDecision`（`query-signals.ts`）保留为结构诊断；⑤ retrievalPlan guard 只做 schema 合法化 / facet 去重 / canonicalize（**保留** LLM 非空 `searchQuery`）。
...
| `retrieve.ts` | 列举已迁至 `corpus-lister/`；KM 不再扫盘补简历 |
...
#### 2.5.11 公司年限 vs 总从业 / 出生年 vs 周岁（✅ P0-32 · 2026-08）
...
| 优先级 | 对策 | 说明 |
|--------|------|------|
| P0 | tenure 按 **槽 searchQuery 实体** 匹配经历区间；有结束日则止于结束日，否则 asOf | `compute-tenure.ts`：searchQuery 去掉 tenure 检索模板后与 excerpt 匹配 |
| P0 | Intake / schema：总从业 vs 单雇主年限（searchQuery 含雇主则分 facet；`birthYear` vs `age`） | prompt few-shot + identityField 枚举 |
| +1 | 出生年问法 → `identityField=birthYear` 抽年份；「多大」→ `compute_age_from_hits`；终稿附 `asOf` | 避免「问年份答周岁」 |
| +1 | Golden / diagnose：复合「某司年限」断言区间或年数与语料一致 | 单测 `compute-tenure.test.ts` |

**验证：** `pnpm --filter @fambrain/brain-service exec vitest run tests/tools/compute-tenure.test.ts tests/tools/compute-age.test.ts tests/tool-orchestrator/field-catalog.test.ts tests/intake-coordinator/repair-retrieval-plan.test.ts`；`verify:orchestrated-identity`。
...
| 3 | 「不止这一个」 | 若 LLM 标 **clarify** → 信 clarify（消解靠 node merge）；若 **retrieve** 且 plan 未写齐 → prompt 约束 LLM 补 plan | pipeline **不**发明 continuation 补 prior |
...
| P0 | **`applyIntakeLinkLookupGuard`**：顶层 `external_link` 时 **harmonize** 误标 enumeration；保留 enum+link 混合 plan；空 searchQuery 填模板 | `guards/intake-link-lookup-guard.ts`、`composite/composite-slot-queries.ts` |
...
| P0 | **`applyIntakeContinuationGuard`**：**恒 noop**（指代靠 intake-node merge + LLM） | `guards/intake-continuation-guard.ts` |
| P0 | **`query-signals.ts` 仅结构诊断**（编号行、multipart）；**不**驱动 guard 收束/展开；`decisionRequestsExternalLink()` 读 LLM `queryType` | `query-signals.ts` |
| P0 | **LLM 写齐 plan**：编号多问、stale inherited plan 由 prompt few-shot 约束，**不**在 link/composite guard 发明槽 | `contract/prompt.ts` |
...
**Guard 链顺序（Intake pipeline）：** parse → **continuation noop** → clarify 早退 → chitchat → userFact 早退 → **link lookup**（harmonize only）→ retrievalPlan（合法化 / 去重 / canonicalize）→ composite（**只编译 plan**；空→clarify）→ **`applyEnumerationSlotGuard`**（per-slot 列举 executor · P0-26）。详见 [02-agent-flows §2](./02-agent-flows.md#2-intakecoordinator--入口接线员-)、[§2.5.3](./04-pitfalls.md#253-p0-15--r6-3--composite-分槽检索-2026-06)、[§2.5.10 列举 per-slot](./04-pitfalls.md#2510-列举执行-per-slot-架构升级-p0-26--2026-07)。
...
**日志：** `guard_对外链接`（link lookup reason）；composite 槽 `queryType=external_link`。
...
| `retrieval-node.ts` | composite **按槽**：`km_retrieve` → hybrid；`list_corpus` → `corpus-lister/fetchListSlot` |
| `corpus-lister/` | 纯 list 图节点 `listRetriever`；目录扫盘分页从 KM 迁出 |
...
**覆盖：** G1 闲聊 · G2 姓名 · G3 项目技术 · G4 城管 · G5 无上下文 clarify · G5b 多轮指代 · **G5c 实体替换续问** · **GMem** 跨会话 QQ。
